...
(function () {
  ...
  function normalizeData(src) {
    const out = [];
    // Expect object keyed by category with rows: {english,spanish,phonetic}
  ...
  function recomputeDeck() {
    ...
    deck = allWords.filter(w => active.has(w.cat));
    deckIdx = 0;
    flipped = false;
  }
  ...
  function wireSettings() {
    ...
    const modal   = byId("settingsModal"); // <dialog>
  ...
  // dialog helpers with fallback (for webviews without <dialog> support)
  ...
  function speakCurrent() {
    ...
    if (!w) return;
    try {
      const u = new SpeechSynthesisUtterance(w.es);
      u.lang = "es-ES";
